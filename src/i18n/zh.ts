...
const zh: LANG_MESSAGE = {
  ...
  importFromFile: '从文件导入设置',
  importFromUrl: '从 URL 导入设置',
  uploadSettingsSuccess: '设置上传成功',
  uploadSettingsIconReflectListRemoved:
    '自定义图标数据过长，上传时已自动移除这部分内容，请精简后重试。',
  syncSettingsSuccess: '设置下载成功',
  syncSettingsConfirm: '核心中存储的设置将覆盖以下本地设置：\n\n{keys}\n\n是否应用？',
  importSettingsConfirm: '导入的设置将覆盖以下本地设置：\n\n{keys}\n\n是否应用？',
  dontAskAgainAlwaysApply: '不再提示，以后自动应用',
  confirmBeforeOverride: '覆盖本地设置前确认',
  deleteUploadedSettingsSuccess: '已上传的设置删除成功',
  deleteUploadedSettingsConfirm: '确定要删除核心中已上传的设置吗？此操作不可撤销。',
  upgradeSuccess: '升级成功',
  ...
  importFromBackendTip:
    '默认的 ./zashboard-settings.json 位于 ui 文件夹下，导入前请确保 ui 文件夹下存在该设置文件。',
  importFailed: '导入失败，请检查 URL {url}',
  autoImportFromUrl: '自动从 URL 导入设置',
  autoImportFromUrlTip:
    '启用后，每次打开 zashboard 时会自动从 URL 导入设置并计算 hash，如果 hash 与上次导入的不同，则重新导入并刷新页面。',
  getting: '获取中...',
